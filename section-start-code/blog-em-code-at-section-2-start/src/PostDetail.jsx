...
export function PostDetail({ post }) {
  const { data, isError, error, isLoading } = useQuery(["comments", post.id], ()=>fetchComments(post.id), {staleTime: 0});
  ...
  return (
    <>
      ...
      {
        updateMutation.isSuccess && <p style={{color:"green"}}>The post has (not) been updated</p>
      }
      <p>{post.body}</p>
      <h4>Comments</h4>
      {data.map((comment) => (
